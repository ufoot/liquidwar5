      ...
      if (hline_pos >= 0)
        {
          ...
          hline (bmp, x + hline_pos,
                 y + text_height (font) - gui_font_baseline,
                 x + hline_pos + c - 1, color);
        }
    }
  ...
  if (msg == MSG_DRAW)
    {
      ...
      gui_textout_ex (gui_get_screen (), d->dp, d->x + d->w / 2, d->y, fg,
                      d->bg, TRUE);
  ...
  if (msg == MSG_DRAW)
    {
      ...
      gui_textout_ex (gui_get_screen (), d->dp,
                      d->x + d->w - gui_strlen (d->dp), d->y, fg, d->bg,
                      FALSE);
  ...
  switch (msg)
    ...
    case MSG_DRAW:
      ...
      if ((d->flags & D_GOTFOCUS) &&
        ...
        dotted_rect (d->x + 1 + g, d->y + 1 + g, d->x + d->w - 3 + g,
    ...
    case MSG_DRAW:
      ...
      switch (d->d2)
        ...
        case 1:
          ...
          if (d->flags & D_SELECTED)
            rectfill (gui_bmp, centerx - r / 2, centery - r / 2,
                      centerx + r / 2 - 1, centery + r / 2 - 1, fg);
          break;
      ...
      for (; p <= b; p++)
        {
          ...
          textout_ex (gui_bmp, font, buf, d->x + x, d->y, (f) ? d->bg : fg,
                      (f) ? fg : d->bg);
          x += w;
        }
      if (x < d->w)
 ...
 */
void
_handle_scrollable_scroll_click (DIALOG * d, int listsize, int *offset,
                                 int height)
{
  int xx, yy;
  int hh = d->h - 5;
 ...
 */
void
_handle_scrollable_scroll (DIALOG * d, int listsize, int *index, int *offset)
{
 ...
 */
void
_handle_listbox_click (DIALOG * d)
{
  char *sel = d->dp2;
  int listsize, height;
  int i, j;
  ...
  if (i != d->d1)
    {
      ...
      if (i != d->d2)
        rest_callback (CLAMP (10, text_height (font) * 16 - d->h - 1, 100),
                       idle_cb);
    }
  else
    {
  ...
  if (listsize > height)
    {
      ...
      if (d->flags & D_GOTFOCUS)
        {
          dotted_rect (d->x + 1, d->y + 1, d->x + d->w - 14, d->y + d->h - 2,
                       fg_color, bg);
          dotted_rect (d->x + d->w - 12, d->y + 1, d->x + d->w - 2,
                       d->y + d->h - 2, fg_color, bg);
        }
      else
        {
          rect (gui_bmp, d->x + 1, d->y + 1, d->x + d->w - 14,
                d->y + d->h - 2, bg);
          rect (gui_bmp, d->x + d->w - 12, d->y + 1, d->x + d->w - 2,
                d->y + d->h - 2, bg);
        }
  ...
  else
    {
      ...
      if (d->flags & D_GOTFOCUS)
        dotted_rect (d->x + 1, d->y + 1, d->x + d->w - 2, d->y + d->h - 2,
                     fg_color, bg);
      else
        rect (gui_bmp, d->x + 1, d->y + 1, d->x + d->w - 2, d->y + d->h - 2,
              bg);
    }
}
                  ...
                  if ((int) ((unsigned long) d->dp3) < ustrlen (thisitem))
                    {
                      for (a = 0; a < (int) ((unsigned long) d->dp3); a++)
                        {
                          if (utolower (ugetat (thisitem, a)) !=
                              utolower (ugetat (selected, a)))
                            {
                              failure = TRUE;
                              break;
                            }
                      ...
                      if ((!failure)
                          &&
                          (utolower
                           (ugetat (thisitem, (int) (unsigned long) d->dp3))
                           == utolower (c)))
                        {
                          d->d1 = i;
                          ...
                          _handle_scrollable_scroll (d, listsize, &d->d1,
                                                     &d->d2);
                          object_message (d, MSG_DRAW, 0);
                          return D_USED_CHAR;
                        }
  ...
  switch (msg)
    ...
    case MSG_DRAW:
      ...
      _draw_textbox (d->dp, &d->d1, 1, d->d2,
                     ...
                     d->x, d->y, d->w - bar, d->h,
                     (d->flags & D_DISABLED), fg_color, d->bg, gui_mg_color);
...
d_slider_proc (int msg, DIALOG * d, int c)
{
  ...
  switch (msg)
    ...
    case MSG_DRAW:
      ...
      if (d->flags & D_GOTFOCUS)
        dotted_rect (d->x, d->y, d->x + d->w - 1, d->y + d->h - 1, sfg,
                     d->bg);
      break;
