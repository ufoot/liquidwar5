  ...
  for (c = 0; d[c].proc; c++)
    {
      if ((gui_mouse_x () >= d[c].x) && (gui_mouse_y () >= d[c].y) &&
          (gui_mouse_x () < d[c].x + d[c].w)
          && (gui_mouse_y () < d[c].y + d[c].h)
 ...
 */
static int
obj_list_cmp (AL_CONST void *e1, AL_CONST void *e2)
{
 ...
 */
static int
cmp_tab (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2)
{
 ...
 */
static int
cmp_shift_tab (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2)
{
 ...
 */
static int
min_dist (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2, enum axis main_axis,
          int bias)
{
 ...
 */
static int
cmp_right (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2)
{
  int bias;
 ...
 */
static int
cmp_left (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2)
{
  int bias;
 ...
 */
static int
cmp_down (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2)
{
  int bias;
 ...
 */
static int
cmp_up (AL_CONST DIALOG * d1, AL_CONST DIALOG * d2)
{
  int bias;
 ...
 */
static int
move_focus (DIALOG * d, int ascii, int scan, int *focus_obj)
{
...
init_dialog (DIALOG * dialog, int focus_obj)
{
  ...
  if ((c >= 0)
      && ((object_message (dialog + c, MSG_WANTFOCUS, 0)) & D_WANTFOCUS))
    {
      dialog[c].flags |= D_GOTFOCUS;
      player->focus_obj = c;
    }
 ...
 */
static void
check_for_redraw (DIALOG_PLAYER * player)
{
  ...
  if (player->res & D_REDRAW_ALL)
    {
      for (iter = first_active_dialog_player;
           iter != current_active_dialog_player; iter = iter->next)
  ...
  if (player->res & D_REDRAW)
    {
      player->res ^= D_REDRAW;
      player->res |=
        dialog_message (player->dialog, MSG_DRAW, 0, &player->obj);
    }
...
update_dialog (DIALOG_PLAYER * player)
{
  ...
  if (new_mouse_b != player->mouse_b)
    {
      player->res |=
        offer_focus (player->dialog, player->mouse_obj, &player->focus_obj,
                     FALSE);
  ...
  if (player->click_wait)
    {
      ...
      if ((dclick_status == DCLICK_AGAIN) &&
          ...
          (gui_mouse_x () <=
           player->dialog[player->mouse_obj].x +
           player->dialog[player->mouse_obj].w)
          && (gui_mouse_y () <=
              player->dialog[player->mouse_obj].y +
              player->dialog[player->mouse_obj].h))
        {
  ...
  if (player->res & D_WANTFOCUS)
    {
      ...
      player->res |=
        offer_focus (player->dialog, player->obj, &player->focus_obj, FALSE);
    }
  ...
  if ((cascii) || (cscan) || (keypressed ()))
    {
      ...
      player->res |=
        move_focus (player->dialog, cascii, cscan, &player->focus_obj);
    }
...
shutdown_dialog (DIALOG_PLAYER * player)
{
  ...
  for (iter = first_active_dialog_player, prev = 0; iter != 0;
 ...
 */
static char *
split_around_tab (const char *s, char **tok1, char **tok2)
{
  char *buf, *last;
  char tmp[16];
 ...
 */
static int
bar_entry_length (const char *text)
{
  char *buf, *tok1, *tok2;
  int len;
 ...
 */
static void
get_menu_pos (MENU_PLAYER * m, int c, int *x, int *y, int *w)
{
  int c2;
...
draw_menu_item (MENU_PLAYER * m, int c)
{
  ...
  if (ugetc (m->menu[c].text))
    {
      ...
      if (tok2)
        gui_textout_ex (gui_bmp, tok2, x + w - gui_strlen (tok2) - 10, y + 1,
                        fg, bg, FALSE);
  ...
  if (m->menu[c].flags & D_SELECTED)
    {
      line (gui_bmp, x + 1, y + text_height (font) / 2 + 1, x + 3,
  ...
  if (gui_menu_draw_menu)
  ...
  else
    {
      ...
      rect (gui_bmp, m->x, m->y, m->x + m->w - 2, m->y + m->h - 2,
            gui_fg_color);
      vline (gui_bmp, m->x + m->w - 1, m->y + 1, m->y + m->h - 1,
             gui_fg_color);
      hline (gui_bmp, m->x + 1, m->y + m->h - 1, m->x + m->w - 1,
             gui_fg_color);
    }
 ...
 */
static int
menu_mouse_object (MENU_PLAYER * m)
{
  int c;
  int x, y, w;
  ...
  for (c = 0; c < m->size; c++)
    {
      ...
      if ((gui_mouse_x () >= x) && (gui_mouse_x () < x + w) &&
          (gui_mouse_y () >= y)
          && (gui_mouse_y () < y + (text_height (font) + 4)))
 ...
 */
static INLINE int
mouse_in_single_menu (MENU_PLAYER * m)
{
 ...
 */
static int
mouse_in_parent_menu (MENU_PLAYER * m)
{
  int c;
 ...
 */
static void
layout_menu (MENU_PLAYER * m, MENU * menu, int bar, int x, int y, int minw,
             int minh)
{
  char *buf, *tok1, *tok2;
  int extra = 0;
 ...
 */
static int
menu_key_shortcut (int c, AL_CONST char *s)
{
  int d;
...
menu_alt_key (int k, MENU * m)
{
  static unsigned char alt_table[] = {
    ...
    ALLEGRO_KEY_F, ALLEGRO_KEY_G, ALLEGRO_KEY_H, ALLEGRO_KEY_I,
    ALLEGRO_KEY_J, ALLEGRO_KEY_K, ALLEGRO_KEY_L, ALLEGRO_KEY_M, ALLEGRO_KEY_N,
    ALLEGRO_KEY_O, ALLEGRO_KEY_P, ALLEGRO_KEY_Q, ALLEGRO_KEY_R,
    ALLEGRO_KEY_S, ALLEGRO_KEY_T, ALLEGRO_KEY_U, ALLEGRO_KEY_V, ALLEGRO_KEY_W,
    ALLEGRO_KEY_X, ALLEGRO_KEY_Y, ALLEGRO_KEY_Z
  };
...
update_menu (MENU_PLAYER * player)
{
      ...
      if (!mouse_in_single_menu (player))
        {
          if (mouse_in_parent_menu (player->parent)
              || (!player->mouse_button_was_pressed))
            {
              player->ret = -2;
              goto End;
            }
            ...
            case ALLEGRO_KEY_UP:
              if ((((c >> 8) == ALLEGRO_KEY_LEFT) && (player->bar)) ||
                  (((c >> 8) == ALLEGRO_KEY_UP) && (!player->bar)))
                {
                  ...
                  while ((!ugetc (player->menu[c].text))
                         && (c != player->sel));
                  player->sel = c;
                }
              break;
  ...
  if (player->auto_open && (gui_menu_opening_delay >= 0))
    {                           /* menu auto-opening on? */
      if (!mouse_in_single_menu (player))
        {
          if (mouse_in_parent_menu (player->parent))
            {
...
End:
        ...
        {                       /* child menu? */
          if (player->bar)
          ...
          else
            {
              ...
              child_y =
                player->y + (text_height (font) + 4) * player->ret +
                text_height (font) / 4 + 1;
            }

          /* recursively call child menu */
          player->child =
            init_single_menu (player->menu[player->ret].child, player, NULL,
                              FALSE, child_x, child_y, TRUE, 0, 0);
          return TRUE;          /* continue */
        }
      ...
      if (player->parent)
        {
          ...
          if (child_ret == -3)
            {                   /* return caused by mouse movement? */
  ...
  if ((player->bar) && (!gui_mouse_b ()) && (!keypressed ())
      && (!mouse_in_single_menu (player)))
    return FALSE;
...
d_menu_proc (int msg, DIALOG * d, int c)
{
  ...
  switch (msg)
    ...
    case MSG_CLICK:
      ...
      /* initialize the menu */
      active_menu_player =
        init_single_menu (d->dp, NULL, d, TRUE, d->x, d->y, FALSE, d->w,
                          d->h);
      break;
 ...
 */
int
alert3 (AL_CONST char *s1, AL_CONST char *s2, AL_CONST char *s3,
        AL_CONST char *b1, AL_CONST char *b2, AL_CONST char *b3, int c1,
        int c2, int c3)
{
  char tmp[16];
  int avg_w, avg_h;
...
#define SORT_OUT_BUTTON(x) {                                            \
  ...
  alert_dialog[A_S1].w = alert_dialog[A_S2].w = alert_dialog[A_S3].w =
    maxlen - avg_w * 2;
  ...
  if (buttons == 3)
    {
      alert_dialog[b[0]].x =
        alert_dialog[0].x + maxlen / 2 - len1 * 3 / 2 - avg_w;
      alert_dialog[b[2]].x =
        alert_dialog[0].x + maxlen / 2 + len1 / 2 + avg_w;
    }
  else if (buttons == 2)
    {
  ...
  alert_dialog[A_B1].h = alert_dialog[A_B2].h = alert_dialog[A_B3].h =
    avg_h * 2;
 ...
 */
int
alert (AL_CONST char *s1, AL_CONST char *s2, AL_CONST char *s3,
